...
//			}
     ...
     * @return array
     */
    public function agency_reg($username, $mobile, $password, $password2, $chushengriqi, $push_id = 0, $invite = 0)
    {
        if ($invite > 0) {
            $cur_user = Db::name('users')->where('user_id', $invite)->find();
            ...
            $map['user_money'] = '0.00';//可用余额
            $map['pay_points'] = '1000';//给注册会员增加1000可用积分，这里因为注册都是交过680的。
            $map['level']='2'; //默认注册会员为入单会员

            $user_id = Db::name('users')->insertGetId($map);
            if ($user_id === false) {
                return array('status' => -1, 'msg' => '录入失败');
            }
            //他上线代理的代理人数要加1
            user_agent_info($user_id, 1);
            ...
            distribution_money_by_level($user_id);
            return array('status' => 1, 'msg' => '您好，您已录入会员成功', 'result' => $user);
        } else {
            return array('status' => -1, 'msg' => '推荐人有误');
        }
    }
