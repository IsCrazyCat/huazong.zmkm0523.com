...
class CartLogic extends Model
{
    ...
    /**
     * 获取用户的购物车列表 cart页面 没有计算20%现金和80%积分前
     * @param int $selected|是否被用户勾选中的 0 为全部 1为选中  一般没有查询不选中的商品情况
     * @return array
     */
    public function getUserCartList1($selected){
        // 如果用户已经登录则按照用户id查询
        if ($this->user_id) {
    ...
    public function getUserCartList($selected){
        ...
        $cartList = DB::name('Cart')->where($cartWhere)->select();  // 获取购物车商品
        $total_goods_num = $total_price = $cut_fee = $use_pay_integral = 0;//初始化数据。商品总共数量/需支付商品总额/节约金额/需支付积分
        foreach ($cartList as $k => $val) {
            //购买规则：20%现金和80%积分，入单商品除外只能使用现金购买
            if(0!= strcmp($val['goods_name'],'入单商品')){
                $total_cur_money = $val['goods_num'] * $val['member_goods_price'];
...
//                }
        ...
        $total_price = array('total_fee' => $total_price, 'cut_fee' => $cut_fee, 'num' => $total_goods_num,'use_pay_integral' => $use_pay_integral); // 总计
        setcookie('cn', $total_goods_num, null, '/');
        return array('cartList' => $cartList, 'total_price' => $total_price);
    }
